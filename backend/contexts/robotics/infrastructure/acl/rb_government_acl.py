"""ACL: Government robotics / digital government layer to peers (P216-T)."""
...
def to_integration_platform(*, tenant_id: str, connector_ref: str) -> dict[str, Any]:
    return {"tenant_id": tenant_id, "connector_ref": connector_ref, "via_integration_platform": True, "never_duplicate_government_core_logic": True, "peer_ids_only": True}

def to_government(*, tenant_id: str, government_ref: str) -> dict[str, Any]:
    return {"tenant_id": tenant_id, "government_ref": government_ref, "via_government_api": True, "never_duplicate_government_core_logic": True, "peer_ids_only": True}

def to_municipality(*, tenant_id: str, municipality_ref: str) -> dict[str, Any]:
    return {"tenant_id": tenant_id, "municipality_ref": municipality_ref, "via_municipality_api": True, "never_duplicate_municipality_core_logic": True, "peer_ids_only": True}

def to_national_identity(*, tenant_id: str, national_id_ref: str) -> dict[str, Any]:
    return {"tenant_id": tenant_id, "national_id_ref": national_id_ref, "via_national_id_connector": True, "never_replace_identity_platform": True, "peer_ids_only": True}

def to_policy_engine(*, tenant_id: str, policy_ref: str) -> dict[str, Any]:
    return {"tenant_id": tenant_id, "policy_ref": policy_ref, "via_policy_engine": True, "ungated_physical_autonomy_strategy_forbidden": True, "privacy_by_design_required": True, "human_governance_oversight_required": True, "explainable_ai_required": True, "peer_ids_only": True}

def to_workflow(*, tenant_id: str, workflow_ref: str) -> dict[str, Any]:
    return {"tenant_id": tenant_id, "workflow_ref": workflow_ref, "via_workflow": True, "opaque_safety_strategy_forbidden": True, "human_governance_oversight_required": True, "peer_ids_only": True}
...
def to_enterprise_robotics(*, tenant_id: str, government_ref: str) -> dict[str, Any]:
    return {
        "tenant_id": tenant_id,
        "government_ref": government_ref,
        "via_enterprise_robotics": True,
        "module_local_government_platform_forbidden": True,
