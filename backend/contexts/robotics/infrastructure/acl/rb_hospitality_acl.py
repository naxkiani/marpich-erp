"""ACL: Hospitality robotics / smart hotel layer to peers (P216-P)."""
...
def to_hotel_pms(*, tenant_id: str, pms_ref: str) -> dict[str, Any]:
    return {"tenant_id": tenant_id, "pms_ref": pms_ref, "via_pms_api": True, "never_duplicate_hotel_pms_core_logic": True, "peer_ids_only": True}

def to_booking(*, tenant_id: str, booking_ref: str) -> dict[str, Any]:
    return {"tenant_id": tenant_id, "booking_ref": booking_ref, "via_booking_api": True, "never_duplicate_hotel_pms_core_logic": True, "peer_ids_only": True}

def to_policy_engine(*, tenant_id: str, policy_ref: str) -> dict[str, Any]:
    return {"tenant_id": tenant_id, "policy_ref": policy_ref, "via_policy_engine": True, "ungated_physical_autonomy_strategy_forbidden": True, "privacy_by_design_required": True, "human_centered_hospitality_required": True, "peer_ids_only": True}
...
def to_enterprise_robotics(*, tenant_id: str, hospitality_ref: str) -> dict[str, Any]:
    return {
        "tenant_id": tenant_id,
        "hospitality_ref": hospitality_ref,
        "via_enterprise_robotics": True,
        "module_local_hospitality_platform_forbidden": True,
