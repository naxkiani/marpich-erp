"""ACL: Healthcare robotics / medical AI / surgical layer to peers (P216-I)."""
...
def to_master_ai(*, tenant_id: str, master_ai_ref: str) -> dict[str, Any]:
    return {"tenant_id": tenant_id, "master_ai_ref": master_ai_ref, "via_p214_z": True, "never_replace_ai_platform": True, "no_module_local_llm": True, "medical_ai_via_p214z_acl_only": True, "physical_ai_via_p214z_acl_only": True, "peer_ids_only": True}
...
def to_integration_platform(*, tenant_id: str, connector_ref: str) -> dict[str, Any]:
    return {"tenant_id": tenant_id, "connector_ref": connector_ref, "via_integration_platform": True, "fhir_hl7_dicom_via_integration_platform_only": True, "never_direct_fhir_hl7_dicom_bypass": True, "peer_ids_only": True}
...
def to_clinic(*, tenant_id: str, clinic_ref: str) -> dict[str, Any]:
    return {"tenant_id": tenant_id, "clinic_ref": clinic_ref, "via_clinic_api": True, "never_duplicate_hospital_clinic_core_logic": True, "peer_ids_only": True}

def to_policy_engine(*, tenant_id: str, policy_ref: str) -> dict[str, Any]:
    return {"tenant_id": tenant_id, "policy_ref": policy_ref, "via_policy_engine": True, "ungated_physical_autonomy_strategy_forbidden": True, "human_in_the_loop_required_for_clinical_autonomy": True, "peer_ids_only": True}
...
def to_enterprise_robotics(*, tenant_id: str, healthcare_ref: str) -> dict[str, Any]:
    return {
        "tenant_id": tenant_id,
        "healthcare_ref": healthcare_ref,
        "via_enterprise_robotics": True,
        "module_local_healthcare_platform_forbidden": True,
