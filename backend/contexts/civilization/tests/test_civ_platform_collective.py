"""P219-W collective intelligence & global coordination platform foundation tests."""
...
@pytest.mark.unit
def test_civ_collective_foundation():
    result = validate_civ_collective_foundation(repo_root=REPO_ROOT)
    assert result["passed"] is True, result
    assert result["prompt"] == "P219-W"
    assert result["adr"] == 576
    assert result["sor"] == "civilization"
    assert result["capability"] == "CAP-PLT-CIV-001"


@pytest.mark.unit
def test_civ_collective_catalog():
    cat = mod.catalog()
    assert cat["prompt_id"] == "P219-W"
    assert cat["fabric"] == "meos_civilization_os_collective_intelligence_global_coordination_framework"
    ...
    assert cat["architecture"]["community_domain_count"] == 10
    assert cat["architecture"]["knowledge_source_count"] == 8
    assert cat["architecture"]["consensus_lifecycle_step_count"] == 7
    ...
    assert cat["never_replace_p219_v_general_intelligence"] is True
    assert cat["never_replace_p219_i_knowledge"] is True
    assert cat["never_replace_institutional_governance"] is True
    assert cat["never_centralized_autonomous_control_of_collective_intelligence"] is True
    assert cat["never_ungated_collective_decision_execution"] is True
    assert cat["never_opaque_unexplainable_collective_recommendations"] is True
    assert cat["never_bypass_human_accountability_collective_intelligence"] is True
    assert cat["no_module_local_llm"] is True
    assert cat["foundation_for_p219_x"] is True
    assert "GET /civilization/collective-intelligence" in mod.collective_surface()["routes"]
    assert "integrating distributed expertise, institutional knowledge and AI-assisted analysis" in cat["primary_capability"]


@pytest.mark.unit
def test_civ_collective_acl():
    from contexts.civilization.infrastructure.acl import civ_collective_acl as acls
    ...
    assert acls.to_civilization_gen_intel(tenant_id="t1", gen_intel_ref="g1")["never_replace_p219_v_general_intelligence"] is True
    assert acls.to_identity(tenant_id="t1", identity_ref="i1")["never_bypass_human_accountability_collective_intelligence"] is True
    assert acls.to_policy_engine(tenant_id="t1", policy_ref="p1")["never_replace_policy_engine"] is True
    assert acls.to_workflow(tenant_id="t1", workflow_ref="w1")["never_ungated_collective_decision_execution"] is True
    ...
    assert acls.to_enterprise_civilization(tenant_id="t1", collective_ref="c1")["never_bypass_human_accountability_collective_intelligence"] is True
    assert acls.to_core_platform(tenant_id="t1", core_ref="c1")["never_replace_core_platform"] is True


@pytest.mark.unit
@pytest.mark.asyncio
async def test_service_catalog_includes_platform_collective():
    svc = get_civilization_service()
    catalog = (await svc.list_catalog()).unwrap()
    assert catalog["platform_collective"]["prompt_id"] == "P219-W"
    assert catalog["platform_collective"]["foundation_for_p219_x"] is True
    assert svc.collective_readiness()["passed"] is True
