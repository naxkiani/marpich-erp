"""P219-N civilization sustainability intelligence platform foundation tests (unit path)."""
...
@pytest.mark.unit
def test_civ_sustainability_foundation():
    result = validate_civ_sustainability_foundation(repo_root=REPO_ROOT)
    assert result["passed"] is True, result
    assert result["prompt"] == "P219-N"
    assert result["adr"] == 567
    assert result["sor"] == "civilization"
    assert result["capability"] == "CAP-PLT-CIV-001"


@pytest.mark.unit
def test_civ_sustainability_catalog():
    cat = mod.catalog()
    assert cat["prompt_id"] == "P219-N"
    assert cat["fabric"] == "meos_civilization_os_civilization_sustainability_intelligence_framework"
    ...
    assert cat["architecture"]["environment_domain_count"] == 10
    assert cat["architecture"]["platform_domain_count"] == 10
    assert cat["architecture"]["circular_domain_count"] == 8
    assert cat["architecture"]["carbon_domain_count"] == 6
    assert cat["agents"]["agent_count"] == 5
    assert cat["knowledge_graph"]["entity_count"] == 12
    assert cat["knowledge_graph"]["relationship_count"] == 9
    assert cat["digital_twin"]["twin_count"] == 8
    ...
    assert cat["never_replace_p219_m_security"] is True
    assert cat["never_replace_p219_f_simulation"] is True
    assert cat["never_replace_p219_g_resources"] is True
    assert cat["never_ungated_environmental_policy_execution"] is True
    assert cat["never_treat_climate_forecast_as_binding_policy"] is True
    ...
    assert cat["foundation_for_p219_o"] is True
    assert "GET /civilization/sustainability" in mod.sustainability_surface()["routes"]
    assert "continuously monitoring, predicting and optimizing environmental, ecological" in cat["primary_capability"]


@pytest.mark.unit
def test_civ_sustainability_acl():
    from contexts.civilization.infrastructure.acl import civ_sustainability_acl as acls
    ...
    assert acls.to_identity(tenant_id="t1", identity_ref="i1")["never_violate_human_sovereignty_sustainability"] is True
    assert acls.to_policy_engine(tenant_id="t1", policy_ref="p1")["never_replace_policy_engine"] is True
    assert acls.to_policy_engine(tenant_id="t1", policy_ref="p1")["never_treat_climate_forecast_as_binding_policy"] is True
    assert acls.to_workflow(tenant_id="t1", workflow_ref="w1")["never_replace_workflow"] is True
    assert acls.to_workflow(tenant_id="t1", workflow_ref="w1")["never_ungated_environmental_policy_execution"] is True
    ...
    assert acls.to_enterprise_civilization(tenant_id="t1", sustainability_ref="s1")["never_bypass_circular_economy_safeguards"] is True
...
async def test_service_catalog_includes_platform_sustainability():
    ...
    assert catalog["platform_sustainability"]["prompt_id"] == "P219-N"
    assert catalog["platform_sustainability"]["foundation_for_p219_o"] is True
