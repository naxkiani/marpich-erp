"""P219-M civilization security intelligence platform foundation tests (unit path)."""
...
@pytest.mark.unit
def test_civ_security_foundation():
    result = validate_civ_security_foundation(repo_root=REPO_ROOT)
    assert result["passed"] is True, result
    assert result["prompt"] == "P219-M"
    assert result["adr"] == 566
    assert result["sor"] == "civilization"
    assert result["capability"] == "CAP-PLT-CIV-001"


@pytest.mark.unit
def test_civ_security_catalog():
    cat = mod.catalog()
    assert cat["prompt_id"] == "P219-M"
    assert cat["fabric"] == "meos_civilization_os_civilization_security_intelligence_framework"
    ...
    assert cat["architecture"]["risk_domain_count"] == 10
    assert cat["architecture"]["threat_category_count"] == 8
    assert cat["architecture"]["resilience_domain_count"] == 9
    assert cat["architecture"]["resilience_lifecycle_step_count"] == 7
    ...
    assert cat["never_replace_p219_l_innovation"] is True
    assert cat["never_replace_identity"] is True
    assert cat["never_ungated_autonomous_security_response"] is True
    assert cat["never_bypass_zero_trust_controls"] is True
    assert cat["never_opaque_unexplainable_security_decisions"] is True
    assert cat["no_module_local_llm"] is True
    assert cat["foundation_for_p219_n"] is True
    assert "GET /civilization/security" in mod.security_surface()["routes"]
    assert "protecting planetary infrastructure, human civilization, critical knowledge" in cat["primary_capability"]


@pytest.mark.unit
def test_civ_security_acl():
    from contexts.civilization.infrastructure.acl import civ_security_acl as acls
    ...
    assert acls.to_identity(tenant_id="t1", identity_ref="i1")["never_bypass_zero_trust_controls"] is True
    ...
    assert acls.to_workflow(tenant_id="t1", workflow_ref="w1")["never_ungated_autonomous_security_response"] is True
    ...
    assert acls.to_enterprise_civilization(tenant_id="t1", security_ref="s1")["never_bypass_zero_trust_controls"] is True
...
async def test_service_catalog_includes_platform_security():
    ...
    assert catalog["platform_security"]["prompt_id"] == "P219-M"
    assert catalog["platform_security"]["foundation_for_p219_n"] is True
