"""P219-O civilization prosperity intelligence platform foundation tests (unit path)."""
...
@pytest.mark.unit
def test_civ_prosperity_foundation():
    result = validate_civ_prosperity_foundation(repo_root=REPO_ROOT)
    assert result["passed"] is True, result
    assert result["prompt"] == "P219-O"
    assert result["adr"] == 568
    assert result["sor"] == "civilization"
    assert result["capability"] == "CAP-PLT-CIV-001"


@pytest.mark.unit
def test_civ_prosperity_catalog():
    cat = mod.catalog()
    assert cat["prompt_id"] == "P219-O"
    assert cat["fabric"] == "meos_civilization_os_civilization_prosperity_intelligence_framework"
    ...
    assert cat["architecture"]["human_prosperity_domain_count"] == 10
    assert cat["architecture"]["quality_of_life_dimension_count"] == 10
    assert cat["architecture"]["flourishing_dimension_count"] == 10
    assert cat["architecture"]["opportunity_domain_count"] == 8
    assert cat["architecture"]["optimization_level_count"] == 7
    assert cat["architecture"]["optimization_engine_step_count"] == 7
    ...
    assert cat["never_replace_p219_n_sustainability"] is True
    assert cat["never_replace_p219_h_economy"] is True
    assert cat["never_replace_p219_j_human"] is True
    assert cat["never_ungated_prosperity_optimization_execution"] is True
    assert cat["never_treat_prosperity_score_as_binding_policy"] is True
    assert cat["never_bypass_opportunity_equality_safeguards"] is True
    assert cat["never_opaque_unexplainable_prosperity_decisions"] is True
    assert cat["no_module_local_llm"] is True
    assert cat["foundation_for_p219_p"] is True
    assert "GET /civilization/prosperity" in mod.prosperity_surface()["routes"]
    assert "continuously improving quality of life, opportunity, wellbeing and human" in cat["primary_capability"]


@pytest.mark.unit
def test_civ_prosperity_acl():
    from contexts.civilization.infrastructure.acl import civ_prosperity_acl as acls
    ...
    assert acls.to_identity(tenant_id="t1", identity_ref="i1")["never_violate_human_sovereignty_prosperity"] is True
    assert acls.to_policy_engine(tenant_id="t1", policy_ref="p1")["never_replace_policy_engine"] is True
    assert acls.to_policy_engine(tenant_id="t1", policy_ref="p1")["never_treat_prosperity_score_as_binding_policy"] is True
    assert acls.to_workflow(tenant_id="t1", workflow_ref="w1")["never_replace_workflow"] is True
    assert acls.to_workflow(tenant_id="t1", workflow_ref="w1")["never_ungated_prosperity_optimization_execution"] is True
    ...
    assert acls.to_enterprise_civilization(tenant_id="t1", prosperity_ref="p1")["never_bypass_opportunity_equality_safeguards"] is True
    assert acls.to_core_platform(tenant_id="t1", core_ref="c1")["never_replace_core_platform"] is True


@pytest.mark.unit
@pytest.mark.asyncio
async def test_service_catalog_includes_platform_prosperity():
    svc = get_civilization_service()
    catalog = (await svc.list_catalog()).unwrap()
    assert catalog["platform_prosperity"]["prompt_id"] == "P219-O"
    assert catalog["platform_prosperity"]["foundation_for_p219_p"] is True
    assert svc.prosperity_readiness()["passed"] is True
