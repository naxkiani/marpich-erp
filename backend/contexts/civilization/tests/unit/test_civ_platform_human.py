"""P219-J civilization human intelligence platform foundation tests (unit path)."""
...
@pytest.mark.unit
def test_civ_human_foundation():
    result = validate_civ_human_foundation(repo_root=REPO_ROOT)
    assert result["passed"] is True, result
    assert result["prompt"] == "P219-J"
    assert result["adr"] == 563
    assert result["sor"] == "civilization"
    assert result["capability"] == "CAP-PLT-CIV-001"


@pytest.mark.unit
def test_civ_human_catalog():
    cat = mod.catalog()
    assert cat["prompt_id"] == "P219-J"
    assert cat["fabric"] == "meos_civilization_os_human_civilization_intelligence_framework"
    ...
    assert cat["architecture"]["human_domain_count"] == 9
    assert cat["architecture"]["twin_component_count"] == 8
    assert cat["architecture"]["capability_domain_count"] == 10
    assert cat["architecture"]["development_dimension_count"] == 8
    assert cat["architecture"]["wellbeing_dimension_count"] == 6
    assert cat["workforce"]["agent_count"] == 4
    assert cat["agents"]["agent_count"] == 4
    ...
    assert cat["never_replace_identity"] is True
    assert cat["never_violate_privacy_by_design"] is True
    assert cat["never_ungated_human_profile_mutation"] is True
    assert cat["never_bypass_trusted_human_twin_validation"] is True
    assert cat["never_opaque_unexplainable_human_decisions"] is True
    assert cat["no_module_local_llm"] is True
    assert cat["foundation_for_p219_k"] is True
    assert "GET /civilization/human" in mod.human_surface()["routes"]
    assert "understanding, modeling and continuously improving human capabilities" in cat["primary_capability"]


@pytest.mark.unit
def test_civ_human_acl():
    from contexts.civilization.infrastructure.acl import civ_human_acl as acls
    ...
    assert acls.to_identity(tenant_id="t1", identity_ref="i1")["never_replace_identity"] is True
    assert acls.to_identity(tenant_id="t1", identity_ref="i1")["never_violate_privacy_by_design"] is True
    ...
    assert acls.to_workflow(tenant_id="t1", workflow_ref="w1")["never_ungated_human_profile_mutation"] is True
    assert acls.to_workflow(tenant_id="t1", workflow_ref="w1")["never_bypass_trusted_human_twin_validation"] is True
    assert acls.to_enterprise_civilization(tenant_id="t1", human_ref="h1")["never_replace_identity"] is True
    assert acls.to_core_platform(tenant_id="t1", core_ref="c1")["never_replace_core_platform"] is True


@pytest.mark.unit
@pytest.mark.asyncio
async def test_service_catalog_includes_platform_human():
    svc = get_civilization_service()
    catalog = (await svc.list_catalog()).unwrap()
    assert catalog["platform_human"]["prompt_id"] == "P219-J"
    assert catalog["platform_human"]["foundation_for_p219_k"] is True
    assert svc.human_readiness()["passed"] is True
