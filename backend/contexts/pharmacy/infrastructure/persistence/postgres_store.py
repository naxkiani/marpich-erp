"""PostgreSQL repositories — Pharmacy bounded context (CAP-HLT-008)."""
...
from shared.infrastructure.database.orm import PharmacyDispenseRecordRow, PharmacyPrescriptionRow
...
class PostgresPrescriptionRepository(IPrescriptionRepository):
    async def save(self, prescription: Prescription) -> None:
        async with session_scope() as session:
            ...
            if row is None:
                ...
                row.source_encounter_ref = prescription.source_encounter_ref
    ...
    async def find_by_id(self, tenant_id: str, prescription_id: UniqueId) -> Prescription | None:
        async with session_scope() as session:
            ...
            return _rx_from_row(row) if row and row.tenant_id == tenant_id else None
    ...
    async def find_by_rx_number(self, tenant_id: str, rx_number: str) -> Prescription | None:
        async with session_scope() as session:
            ...
            return _rx_from_row(row) if row else None
    ...
    async def list_prescriptions(self, tenant_id: str) -> list[Prescription]:
        ...
        return [_rx_from_row(r) for r in rows]


class PostgresDispenseRepository(IDispenseRepository):
    async def save(self, dispense: DispenseRecord) -> None:
        async with session_scope() as session:
            row = await session.get(PharmacyDispenseRecordRow, UUID(str(dispense.id)))
            if row is None:
                session.add(
                    PharmacyDispenseRecordRow(
    ...
    async def list_dispenses(self, tenant_id: str) -> list[DispenseRecord]:
        async with session_scope() as session:
            rows = (
                await session.scalars(
                    select(PharmacyDispenseRecordRow)
                    .where(PharmacyDispenseRecordRow.tenant_id == tenant_id)
                    .order_by(PharmacyDispenseRecordRow.dispensed_at.desc())
                )
            ).all()
        return [_dispense_from_row(r) for r in rows]


def _rx_from_row(row: PharmacyPrescriptionRow) -> Prescription:
    return Prescription(
...
def _dispense_from_row(row: PharmacyDispenseRecordRow) -> DispenseRecord:
