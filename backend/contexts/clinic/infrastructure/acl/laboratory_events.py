"""Clinic ACL — laboratory.result.available → local lab result projection."""
...
def make_laboratory_result_handler(
    get_service: Callable[[], "ClinicApplicationService"],
):
    async def handle_laboratory_result_available(envelope: dict) -> None:
        ...
        if not tenant_id or not event_id:
            logger.warning("clinic lab acl skipped — missing tenant/event_id")
            return
        ...
        if not result.succeeded:
            logger.error("clinic lab acl failed: %s", result.error)
