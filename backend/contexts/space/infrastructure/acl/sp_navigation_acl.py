"""ACL: Space navigation layer to peers (P218-I)."""
...
def to_satellite(*, tenant_id: str, satellite_ref: str) -> dict[str, Any]:
    return {"tenant_id": tenant_id, "satellite_ref": satellite_ref, "via_p218_f": True, "never_replace_p218_f_satellite": True, "peer_ids_only": True}

def to_orbital(*, tenant_id: str, orbital_ref: str) -> dict[str, Any]:
    return {"tenant_id": tenant_id, "orbital_ref": orbital_ref, "via_p218_g": True, "never_replace_p218_g_orbital": True, "peer_ids_only": True}

def to_communications(*, tenant_id: str, communications_ref: str) -> dict[str, Any]:
    return {"tenant_id": tenant_id, "communications_ref": communications_ref, "via_p218_h": True, "never_replace_p218_h_communications": True, "peer_ids_only": True}
...
def to_integration(*, tenant_id: str, integration_ref: str) -> dict[str, Any]:
    return {"tenant_id": tenant_id, "integration_ref": integration_ref, "via_integration": True, "no_module_local_telemetry_stack": True, "no_module_local_gnss_receiver_stack": True, "never_skip_gnss_spoofing_detection": True, "peer_ids_only": True}
...
def to_workflow(*, tenant_id: str, workflow_ref: str) -> dict[str, Any]:
    return {"tenant_id": tenant_id, "workflow_ref": workflow_ref, "via_workflow": True, "never_opaque_mission_critical_strategy": True, "never_ungated_autonomous_mission_strategy": True, "never_ungated_guidance_command": True, "never_disable_human_override": True, "never_skip_human_mission_oversight_strategy": True, "peer_ids_only": True}
...
def to_enterprise_space(*, tenant_id: str, navigation_ref: str) -> dict[str, Any]:
    return {
        "tenant_id": tenant_id, "navigation_ref": navigation_ref,
        "via_enterprise_space": True,
        "module_local_navigation_forbidden": True,
