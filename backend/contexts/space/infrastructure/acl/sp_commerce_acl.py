"""ACL: Commerce intelligence to platform peers (P218-R)."""
...
def to_integration(*, tenant_id: str, integration_ref: str) -> dict[str, Any]:
    return _peer(tenant_id, "integration_ref", integration_ref, via_integration=True, no_module_local_telemetry_stack=True)


def to_financial_kernel(*, tenant_id: str, financial_ref: str) -> dict[str, Any]:
    return _peer(tenant_id, "financial_ref", financial_ref, via_financial_kernel=True, never_duplicate_financial_kernel=True, never_ungated_commercial_transaction=True)
...
def to_audit(*, tenant_id: str, audit_ref: str) -> dict[str, Any]:
    return _peer(tenant_id, "audit_ref", audit_ref, via_audit=True, never_opaque_unexplainable_commerce_decisions=True)


def to_identity(*, tenant_id: str, identity_ref: str) -> dict[str, Any]:
    return _peer(tenant_id, "identity_ref", identity_ref, via_identity=True, never_skip_marketplace_identity_verification=True)
...
def to_enterprise_space(*, tenant_id: str, commerce_ref: str) -> dict[str, Any]:
    return _peer(
        tenant_id, "commerce_ref", commerce_ref,
        via_enterprise_space=True, module_local_commerce_forbidden=True,
