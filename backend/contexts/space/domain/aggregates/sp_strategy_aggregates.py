"""P218-B aggregates — space strategic architecture invariants."""
...
class StrategicArchitectureRoot(AggregateRoot):
    ...
    def enable(cls, *, tenant_id: str, architecture_ref: str, present: bool = True):
        if not present: raise ValueError("space.strategy.space_strategic_architecture_is_missing")
        return _mk(cls, tenant_id, "architecture_ref", architecture_ref, "space.strategy.space_strategic_architecture_is_missing", "SpaceArchitecturePublishedEvent")
...
class CapabilityModelRoot(AggregateRoot):
    ...
    def enable(cls, *, tenant_id: str, capability_ref: str, present: bool = True):
        if not present: raise ValueError("space.strategy.capability_model_is_missing")
        return _mk(cls, tenant_id, "capability_ref", capability_ref, "space.strategy.capability_model_is_missing", "SpaceCapabilityRegisteredEvent")
    def is_missing(self)->bool: return not self.present
...
class OperatingFrameworkRoot(AggregateRoot):
    ...
    def enable(cls, *, tenant_id: str, framework_ref: str, present: bool = True):
        if not present: raise ValueError("space.strategy.operating_framework_is_missing")
        return _mk(cls, tenant_id, "framework_ref", framework_ref, "space.strategy.operating_framework_is_missing", "SpaceOperatingModelUpdatedEvent")
    def is_missing(self)->bool: return not self.present
...
class ServiceModelRoot(AggregateRoot):
    ...
    def enable(cls, *, tenant_id: str, service_ref: str, present: bool = True):
        if not present: raise ValueError("space.strategy.service_model_is_missing")
        return _mk(cls, tenant_id, "service_ref", service_ref, "space.strategy.service_model_is_missing", "SpaceServiceCatalogUpdatedEvent")
    def is_missing(self)->bool: return not self.present
...
class OrganizationalModelRoot(AggregateRoot):
    ...
    def enable(cls, *, tenant_id: str, org_ref: str, present: bool = True):
        if not present: raise ValueError("space.strategy.organizational_model_is_missing")
        return _mk(cls, tenant_id, "org_ref", org_ref, "space.strategy.organizational_model_is_missing", "SpaceArchitecturePublishedEvent")
    def is_missing(self)->bool: return not self.present
...
class GovernanceModelRoot(AggregateRoot):
    ...
    def enable(cls, *, tenant_id: str, governance_ref: str, present: bool = True):
        if not present: raise ValueError("space.strategy.governance_model_is_missing")
        return _mk(cls, tenant_id, "governance_ref", governance_ref, "space.strategy.governance_model_is_missing", "SpaceArchitecturePublishedEvent")
    def is_missing(self)->bool: return not self.present
...
class SecurityModelRoot(AggregateRoot):
    ...
    def enable(cls, *, tenant_id: str, security_ref: str, present: bool = True):
        if not present: raise ValueError("space.strategy.security_model_is_missing")
        return _mk(cls, tenant_id, "security_ref", security_ref, "space.strategy.security_model_is_missing", "SpaceArchitecturePublishedEvent")
    def is_missing(self)->bool: return not self.present
...
class DataArchitectureRoot(AggregateRoot):
    ...
    def enable(cls, *, tenant_id: str, data_ref: str, present: bool = True):
        if not present: raise ValueError("space.strategy.data_architecture_is_missing")
        return _mk(cls, tenant_id, "data_ref", data_ref, "space.strategy.data_architecture_is_missing", "SpaceCapabilityRegisteredEvent")
    def is_missing(self)->bool: return not self.present
...
class MaturityModelRoot(AggregateRoot):
    ...
    def enable(cls, *, tenant_id: str, maturity_ref: str, present: bool = True):
        if not present: raise ValueError("space.strategy.maturity_model_is_missing")
        return _mk(cls, tenant_id, "maturity_ref", maturity_ref, "space.strategy.maturity_model_is_missing", "SpaceMaturityAssessedEvent")
    def is_missing(self)->bool: return not self.present
