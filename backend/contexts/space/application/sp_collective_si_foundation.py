"""Space P218-W Collective Super Intelligence foundation validator."""
...
REQUIRED_ARTIFACTS = [
    "docs/adr/549-enterprise-space-intelligence-collective-si.md",
    "docs/architecture/ENTERPRISE_SPACE_INTELLIGENCE_COLLECTIVE_SI.md",
    "docs/architecture/space/COLLECTIVE_SI_ARCHITECTURE.v1.yaml",
    "docs/architecture/space/COLLECTIVE_SI_LIFECYCLE.v1.yaml",
    "docs/architecture/space/COLLECTIVE_SI_DDD_CQRS.v1.yaml",
    "docs/architecture/space/COLLECTIVE_SI_CONTROLS.v1.yaml",
    "docs/architecture/space/COLLECTIVE_SI_VALIDATION.v1.yaml",
    "docs/architecture/space/P218_MASTER_SERIES_ROADMAP.v1.yaml",
    "backend/contexts/space/domain/services/sp_platform_collective_si.py",
    "backend/contexts/space/domain/aggregates/sp_collective_si_aggregates.py",
    "backend/contexts/space/infrastructure/acl/sp_collective_si_acl.py",
    "backend/contexts/space/application/sp_collective_si_foundation.py",
]
FORBIDDEN_SIBLINGS = (
    "backend/contexts/collective_si_platform",
    "backend/contexts/collective_intelligence_bc",
    "backend/contexts/cognitive_ecosystem_bc",
)


def validate_sp_collective_si_foundation(*, repo_root: Path | None = None) -> dict:
    root = repo_root or REPO_ROOT
    missing = [rel for rel in REQUIRED_ARTIFACTS if not (root / rel).exists()]
    sibling = any((root / path).exists() for path in FORBIDDEN_SIBLINGS)
    from contexts.space.domain.aggregates.sp_collective_si_aggregates import (
        CollectiveIntelligenceTwinRoot, CollectiveReasoningRoot, CollectiveSiCoreRoot,
        CollectiveSiKnowledgeGraphRoot, CsiEthicsRoot, CsiGovernanceRoot,
        GlobalCognitiveEcosystemRoot, HumanAiCivilizationNetworkRoot, IntelligenceAlignmentRoot,
    )
    from contexts.space.domain.services import sp_platform_collective_si as catmod

    cat = catmod.catalog()
    catalog_ok = (
        cat["prompt_id"] == "P218-W" and cat["adr"] == 549 and cat["sor"] == "space"
        and cat["capability"] == "CAP-PLT-SP-001"
        and cat["fabric"] == "meos_collective_intelligence_fabric"
        ...
        and cat["human_gi_gate"] == "P218-V"
        and cat["bio_gate"] == "P217-Z" and cat["robotics_gate"] == "P216-Z"
        and cat["quantum_gate"] == "P215-Z" and cat["ai_gate"] == "P214-Z"
        and cat["collective_super_intelligence_present_required"] is True
        and cat["human_ai_civilization_network_present_required"] is True
        and cat["global_cognitive_ecosystem_present_required"] is True
        and cat["collective_reasoning_architecture_present_required"] is True
        ...
        and cat["collective_si"]["capability_count"] == 7
        and cat["civilization_network"]["architecture_model_count"] == 5
        and cat["civilization_network"]["participant_count"] == 7
        and cat["cognitive_ecosystem"]["domain_count"] == 8
        and cat["collective_reasoning"]["mode_count"] == 6
        and cat["collective_reasoning"]["agent_count"] == 5
        and cat["knowledge_graph"]["entity_count"] == 9
        and cat["knowledge_graph"]["relationship_count"] == 5
        and cat["alignment"]["human_contribution_count"] == 5
        and cat["alignment"]["ai_contribution_count"] == 5
        ...
        and cat["foundation_for_p218_x"] is True
        and cat["production_readiness"]["verdict"] == "ENTERPRISE_GRADE"
    )
    checks = [
        CollectiveSiCoreRoot.enable(tenant_id="t1", core_ref="c1").is_missing() is False,
        HumanAiCivilizationNetworkRoot.enable(tenant_id="t1", network_ref="n1").is_missing() is False,
        GlobalCognitiveEcosystemRoot.enable(tenant_id="t1", ecosystem_ref="e1").is_missing() is False,
        CollectiveReasoningRoot.enable(tenant_id="t1", reasoning_ref="r1").is_missing() is False,
        CollectiveSiKnowledgeGraphRoot.enable(tenant_id="t1", kg_ref="kg1").is_missing() is False,
        CollectiveIntelligenceTwinRoot.enable(tenant_id="t1", twin_ref="tw1").is_missing() is False,
        IntelligenceAlignmentRoot.enable(tenant_id="t1", alignment_ref="a1").is_missing() is False,
        CsiEthicsRoot.enable(tenant_id="t1", ethics_ref="eth1").is_missing() is False,
        CsiGovernanceRoot.enable(tenant_id="t1", governance_ref="g1").is_missing() is False,
    ]
    acl_text = (root / "backend/contexts/space/infrastructure/acl/sp_collective_si_acl.py").read_text(encoding="utf-8")
    acl_ok = all(x in acl_text for x in (
        ...
        "to_human_evolution", "to_human_gi", "to_biotechnology", "to_robotics_supreme",
        ...
        "never_replace_p218_t_civilization", "never_ungated_collective_decision",
        ...
        "never_skip_human_authority_preservation", "module_local_collective_si_forbidden",
    ...
    router_ok = all(x in router for x in (
        '@space_router.get("/collective-si")', "/collective-si/vision",
        "/collective-si/architecture", "/collective-si/lifecycle",
        "/collective-si/core", "/collective-si/civilization-network",
        "/collective-si/cognitive-ecosystem", "/collective-si/reasoning",
        "/collective-si/knowledge-graph", "/collective-si/digital-twin",
        "/collective-si/alignment", "/collective-si/ethics",
        "/collective-si/governance", "/collective-si/observability",
        "/collective-si/security", "/collective-si/integration",
        "/collective-si/deployment", "/collective-si/testing",
        "/collective-si/cqrs", "/collective-si/events",
        "/collective-si/readiness",
    ))
    law = (root / "docs/architecture/ENTERPRISE_SPACE_INTELLIGENCE_COLLECTIVE_SI.md").read_text(encoding="utf-8")
    doc_ok = all(x in law for x in (
        "Never Collective Super Intelligence is missing",
        "Never Human-AI Civilization Network is missing",
        "Never Global Cognitive Ecosystem is missing",
        "Never Collective Reasoning Architecture is missing",
        ...
        "global intelligence ecosystem where humans, AI systems",
        "P218-W", "P218-X",
    ))
    passed = not missing and not sibling and catalog_ok and all(checks) and acl_ok and router_ok and doc_ok
    return {
        "prompt": "P218-W", "adr": 549, "passed": passed,
