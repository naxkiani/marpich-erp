"""P5.1 Document Exchange — RSA-PSS-SHA256 content sign + public QR verify."""
...
from contexts.documents.infrastructure.crypto.pem_rsa_signer import PemRsaPssContentSigner
...
async def _auth_headers(client: AsyncClient, tenant: str) -> dict[str, str]:
    await client.post(
        ...
        json={"email": "docs-p51@dev.io", "password": "SecurePass123!", "display_name": "Docs Admin"},
        headers={"X-Tenant-ID": tenant},
    )
    login = await client.post(
        "/api/v1/auth/login",
        json={"email": "docs-p51@dev.io", "password": "SecurePass123!"},
        headers={"X-Tenant-ID": tenant},
    )
...
@pytest.mark.asyncio
async def test_sign_rsa_pss_and_public_verify(client):
    slug = "docs-p51-rsa"
    await client.post(
        "/api/v1/platform/tenants",
        json={"name": "RSA Co", "slug": slug, "industry_pack": "university"},
    )
    ...
    created = await client.post(
        ...
        json={
            ...
            "file_name": "cert.txt",
            "content": "Official certificate body — P5.1",
        },
        headers=headers,
    )
    ...
    assert body["key_id"]
    assert body["evidence"]["algorithm"] == "RSA-PSS-SHA256"
    assert body["evidence"]["hsm"] is False
...
@pytest.mark.asyncio
async def test_tampered_rsa_signature_fails_crypto_verify():
    signer = PemRsaPssContentSigner()
    evidence = signer.sign(
        document_id="doc-1",
        version_checksum="abc",
        signer_id="user-1",
    )
    assert evidence["algorithm"] == "RSA-PSS-SHA256"
    ...
    assert not signer.verify(
        ...
        version_checksum="abc",
        signer_id="user-1",
        signature=evidence["signature"][:-4] + "AAAA",
        algorithm=evidence["algorithm"],
        key_id=evidence["key_id"],
    )
    assert not signer.verify(
        document_id="doc-1",
        version_checksum="TAMPERED",
        signer_id="user-1",
        signature=evidence["signature"],
        algorithm=evidence["algorithm"],
        key_id=evidence["key_id"],
