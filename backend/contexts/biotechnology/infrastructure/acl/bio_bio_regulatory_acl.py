"""ACL: Biotechnology Bio Regulatory layer to peers (P217-N)."""
...
def to_biotechnology_simulation(*, tenant_id: str, simulation_ref: str) -> dict[str, Any]:
    return {
        ...
        "regulatory_twins_via_p217g_acl_only": True, "peer_ids_only": True,
    }
...
def to_biotechnology_drug_discovery(*, tenant_id: str, drug_discovery_ref: str) -> dict[str, Any]:
    return {
        ...
        "drug_regulatory_via_p217k_acl_only": True, "peer_ids_only": True,
    }

def to_biotechnology_bio_manufacturing(*, tenant_id: str, manufacturing_ref: str) -> dict[str, Any]:
    return {
        "tenant_id": tenant_id, "manufacturing_ref": manufacturing_ref,
        "via_p217_l": True, "never_replace_p217_l_bio_manufacturing": True,
        "manufacturing_compliance_via_p217l_acl_only": True, "peer_ids_only": True,
    }

def to_biotechnology_bio_supply_chain(*, tenant_id: str, supply_ref: str) -> dict[str, Any]:
    return {
        "tenant_id": tenant_id, "supply_ref": supply_ref,
        "via_p217_m": True, "never_replace_p217_m_bio_supply_chain": True,
        "distribution_compliance_via_p217m_acl_only": True, "peer_ids_only": True,
    }
...
def to_workflow(*, tenant_id: str, workflow_ref: str) -> dict[str, Any]:
    return {
        ...
        "never_skip_explainable_regulatory_ai": True, "peer_ids_only": True,
    }

def to_audit(*, tenant_id: str, audit_ref: str) -> dict[str, Any]:
    return {
        "tenant_id": tenant_id, "audit_ref": audit_ref, "via_audit": True,
        "never_opaque_unexplainable_decisions": True,
        "immutable_audit_trails_required": True, "peer_ids_only": True,
    }
...
def to_compliance(*, tenant_id: str, compliance_ref: str) -> dict[str, Any]:
    return {"tenant_id": tenant_id, "compliance_ref": compliance_ref, "never_replace_compliance_platform": True, "peer_ids_only": True}
...
def to_enterprise_biotechnology(*, tenant_id: str, bio_regulatory_ref: str) -> dict[str, Any]:
    return {
        "tenant_id": tenant_id, "bio_regulatory_ref": bio_regulatory_ref,
        "via_enterprise_biotechnology": True,
        "module_local_biotechnology_bio_regulatory_forbidden": True,
