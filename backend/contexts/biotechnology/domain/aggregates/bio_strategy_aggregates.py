"""P217-B aggregates — biotechnology strategic architecture invariants."""
...
class StrategicArchitectureRoot(AggregateRoot):
    ...
    def enable(cls, *, tenant_id: str, architecture_ref: str, present: bool = True):
        if not present: raise ValueError("biotechnology.strategy.biotechnology_strategic_architecture_is_missing")
        return _mk(cls, tenant_id, "architecture_ref", architecture_ref, "biotechnology.strategy.biotechnology_strategic_architecture_is_missing", "BioArchitecturePublishedEvent")
...
class CapabilityModelRoot(AggregateRoot):
    ...
    def enable(cls, *, tenant_id: str, capability_ref: str, present: bool = True):
        if not present: raise ValueError("biotechnology.strategy.capability_model_is_missing")
        return _mk(cls, tenant_id, "capability_ref", capability_ref, "biotechnology.strategy.capability_model_is_missing", "BioCapabilityRegisteredEvent")
    def is_missing(self)->bool: return not self.present
...
class OperatingFrameworkRoot(AggregateRoot):
    ...
    def enable(cls, *, tenant_id: str, framework_ref: str, present: bool = True):
        if not present: raise ValueError("biotechnology.strategy.operating_framework_is_missing")
        return _mk(cls, tenant_id, "framework_ref", framework_ref, "biotechnology.strategy.operating_framework_is_missing", "BioOperatingModelUpdatedEvent")
    def is_missing(self)->bool: return not self.present
...
class ServiceModelRoot(AggregateRoot):
    ...
    def enable(cls, *, tenant_id: str, service_ref: str, present: bool = True):
        if not present: raise ValueError("biotechnology.strategy.service_model_is_missing")
        return _mk(cls, tenant_id, "service_ref", service_ref, "biotechnology.strategy.service_model_is_missing", "BioServiceCatalogUpdatedEvent")
    def is_missing(self)->bool: return not self.present
...
class OrganizationalModelRoot(AggregateRoot):
    ...
    def enable(cls, *, tenant_id: str, org_ref: str, present: bool = True):
        if not present: raise ValueError("biotechnology.strategy.organizational_model_is_missing")
        return _mk(cls, tenant_id, "org_ref", org_ref, "biotechnology.strategy.organizational_model_is_missing", "BioArchitecturePublishedEvent")
    def is_missing(self)->bool: return not self.present
...
class GovernanceModelRoot(AggregateRoot):
    ...
    def enable(cls, *, tenant_id: str, governance_ref: str, present: bool = True):
        if not present: raise ValueError("biotechnology.strategy.governance_model_is_missing")
        return _mk(cls, tenant_id, "governance_ref", governance_ref, "biotechnology.strategy.governance_model_is_missing", "BioArchitecturePublishedEvent")
    def is_missing(self)->bool: return not self.present
...
class SecurityModelRoot(AggregateRoot):
    ...
    def enable(cls, *, tenant_id: str, security_ref: str, present: bool = True):
        if not present: raise ValueError("biotechnology.strategy.security_model_is_missing")
        return _mk(cls, tenant_id, "security_ref", security_ref, "biotechnology.strategy.security_model_is_missing", "BioArchitecturePublishedEvent")
    def is_missing(self)->bool: return not self.present
...
class DataArchitectureRoot(AggregateRoot):
    ...
    def enable(cls, *, tenant_id: str, data_ref: str, present: bool = True):
        if not present: raise ValueError("biotechnology.strategy.data_architecture_is_missing")
        return _mk(cls, tenant_id, "data_ref", data_ref, "biotechnology.strategy.data_architecture_is_missing", "BioCapabilityRegisteredEvent")
    def is_missing(self)->bool: return not self.present
...
class MaturityModelRoot(AggregateRoot):
    ...
    def enable(cls, *, tenant_id: str, maturity_ref: str, present: bool = True):
        if not present: raise ValueError("biotechnology.strategy.maturity_model_is_missing")
        return _mk(cls, tenant_id, "maturity_ref", maturity_ref, "biotechnology.strategy.maturity_model_is_missing", "BioMaturityAssessedEvent")
    def is_missing(self)->bool: return not self.present
